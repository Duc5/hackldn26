  ...
  useEffect(() => {
    ...
    const fetchCore = async () => {
      try {
        ...
        const [summaryRes, roomsRes] = await Promise.all([fetch(`${base}/api/analytics/summary`), fetch(`${base}/api/rooms`)]);
  ...
  useEffect(() => {
    ...
    const fetchSeries = async () => {
      try {
        ...
        const response = await fetch(`${base}/api/analytics/db/rooms/${selectedRoomId}/series?days=${days}&limit=1500`);
        if (!response.ok) {
      ...
      <div className="card dashboard-header">
        <div>
          ...
          <p className="subtitle">
            Track occupancy, comfort, and noise trends across all rooms. Toggle a room to see two-week historical patterns.
          </p>
        </div>
      ...
      <div className="dashboard-grid">
        <div className="card stat-grid">
          <div className="stat-card">
            ...
            <span className="muted">{summary ? `${summary.occupied_desks}/${summary.total_desks} desks` : "-"}</span>
          </div>
          <div className="stat-card">
        ...
        <div className="card trend-card">
          <div className="trend-head">
            <div>
              ...
              <p className="muted">
                {selectedRoom
                  ? `${formatPercent(selectedRoom.occupancy_pct)} occupied · ${formatNumber(selectedRoom.avg_noise_db)} dB · ${formatNumber(selectedRoom.avg_temp_c)}°C`
                  : "No room selected"}
              </p>
            </div>
            <div className="trend-controls">
              <div className="segmented">
                {(["occupancy_pct", "avg_noise_db", "avg_temp_c"] as MetricKey[]).map((key) => (
                  <button key={key} className={metric === key ? "active" : ""} onClick={() => setMetric(key)} type="button">
          ...
          <div className="trend-body">
            {seriesLoading ? <span className="muted">Loading series...</span> : <Sparkline points={series} metric={metric} />}
                ...
                <strong>
                  {series.length
                    ? metric === "occupancy_pct"
                      ...
                      : `${formatNumber(series[series.length - 1][metric])}${metric === "avg_temp_c" ? "°C" : " dB"}`
                    : "-"}
                </strong>
              </div>
              <div>
